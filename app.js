...
const STORAGE_KEY = 'yhi_ledger_v1';
const SYNC_KEY = 'yhi_ledger_sync_v1';
const FILE_NAME = 'YHI_가계부_데이터.json';
const GIST_FILENAME = 'yhi_ledger.json';
...
function saveData(silent, opts){
  ...
  if(!opts.fromRemote){
    ...
    DATA.updated = DATA.updatedAt.slice(0,10);
  }
  try{
...
function renderDash(){
  ...
  }).join('') || '<div class="empty">아직 거래가 없습니다 — 우측 하단 + 버튼으로 첫 거래를 추가하세요</div>';
...
async function createGist(token, data){
  return ghFetch('https://api.github.com/gists', {
    method:'POST',
    body: JSON.stringify({
      description:'YHI 가계부 자동 동기화 (private)',
      public:false,
...
// ===== 📱 모바일 클라우드 동기화 (본인 동기화 설정 기준) =====
async function mobileSyncFromGist(){
  const cfg = getSyncConfig();
  if(!(cfg.enabled && cfg.token && cfg.gistId)){
    toast('먼저 아래에서 클라우드 동기화를 설정하세요', 'err');
    return;
  }
  ...
  try{
    await doPull();
    toast('✅ 동기화 완료! ' + DATA.transactions.length + '건', 'ok');
    if(btn){ btn.textContent = '✅ ' + DATA.transactions.length + '건 완료'; }
...
let excelDefaults = {}; // {type:'지출', user:'...', method:'...', cat:'...'}
